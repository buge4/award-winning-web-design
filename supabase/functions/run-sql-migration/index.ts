...
DECLARE
  v_alloc record; v_prize_pool numeric; v_house_fee numeric;
  v_social numeric; v_jackpot numeric; v_burn numeric;
BEGIN
  ...
  ELSE
    ...
    v_burn := p_gross_amount - v_prize_pool - v_house_fee - v_social - v_jackpot;
  END IF;
  IF v_prize_pool > 0 THEN INSERT INTO ledger_allocations (ledger_event_id, recipient_type, amount, description) VALUES (p_ledger_event_id, 'rollover', v_prize_pool, 'Prize pool allocation'); END IF;
  IF v_house_fee > 0 THEN INSERT INTO ledger_allocations (ledger_event_id, recipient_type, amount, description) VALUES (p_ledger_event_id, 'platform', v_house_fee, 'Platform fee'); END IF;
  IF v_social > 0 THEN INSERT INTO ledger_allocations (ledger_event_id, recipient_type, amount, description) VALUES (p_ledger_event_id, 'social_L1', v_social, 'Social circle allocation'); END IF;
  IF v_jackpot > 0 THEN
    INSERT INTO ledger_allocations (ledger_event_id, recipient_type, amount, description) VALUES (p_ledger_event_id, 'jackpot', v_jackpot, 'Jackpot contribution');
    UPDATE jackpots SET current_balance = current_balance + v_jackpot, updated_at = now() WHERE project = 'auction' AND status = 'ACTIVE';
    INSERT INTO jackpot_contributions (jackpot_id, ledger_event_id, amount) SELECT j.id, p_ledger_event_id, v_jackpot FROM jackpots j WHERE j.project = 'auction' AND j.status = 'ACTIVE' LIMIT 1;
  END IF;
  IF v_burn > 0 THEN
    INSERT INTO ledger_allocations (ledger_event_id, recipient_type, amount, description) VALUES (p_ledger_event_id, 'burn', v_burn, 'Token burn');
    INSERT INTO token_burns (amount, source, ledger_event_id, reference_id) VALUES (v_burn, 'auction_bid', p_ledger_event_id, p_game_instance_id);
  END IF;
...
const SQL_PLACE_AUCTION_BID = `
CREATE OR REPLACE FUNCTION place_auction_bid(
  p_user_id uuid, p_instance_id uuid, p_bid_amount numeric
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $fn$
DECLARE
  v_instance record; v_config record; v_wallet record;
  v_bid_fee numeric; v_ledger_event_id uuid; v_bid_id uuid;
  v_is_duplicate boolean := false; v_existing_bid_id uuid;
  v_bid_position integer; v_split_result jsonb; v_total_bids_after integer;
BEGIN
  SELECT * INTO v_instance FROM auction_instances WHERE id = p_instance_id FOR UPDATE;
  IF v_instance IS NULL THEN RETURN jsonb_build_object('success', false, 'message', 'Auction not found'); END IF;
  ...
  IF v_instance.status = 'accumulating' AND v_config.auction_type = 'live_before_hot' AND v_config.total_bids_to_hot IS NOT NULL AND v_total_bids_after >= v_config.total_bids_to_hot THEN
    UPDATE auction_instances SET status = 'hot_mode', hot_mode_started_at = now(),
  ...
  IF v_instance.status = 'hot_mode' AND v_instance.hot_mode_ends_at IS NOT NULL THEN
    IF v_instance.hot_mode_ends_at - now() < interval '30 seconds' THEN
      UPDATE auction_instances SET hot_mode_ends_at = hot_mode_ends_at + interval '30 seconds',
        scheduled_end = hot_mode_ends_at + interval '30 seconds', status = 'grace_period'
      WHERE id = p_instance_id;
    END IF;
  END IF;
...
const SQL_ADMIN_CREATE_AUCTION = `
CREATE OR REPLACE FUNCTION admin_create_auction(
  p_admin_id uuid, p_name text, p_slug text,
  p_auction_type text DEFAULT 'timed', p_bid_fee numeric DEFAULT 10,
  p_max_bid_value numeric DEFAULT 99.99, p_min_bid_value numeric DEFAULT 0.01,
  p_duration_seconds int DEFAULT 86400, p_image_url text DEFAULT NULL,
  p_total_bids_to_hot int DEFAULT NULL, p_hot_mode_duration_seconds int DEFAULT NULL,
  p_total_bids_to_close int DEFAULT NULL, p_prize_type text DEFAULT 'pool_funded',
  p_manual_prize_title text DEFAULT NULL, p_jackpot_seed numeric DEFAULT 0,
  p_split_prize_pct numeric DEFAULT 55, p_split_burn_pct numeric DEFAULT 15,
  p_split_house_pct numeric DEFAULT 15, p_split_social_pct numeric DEFAULT 5,
  p_split_jackpot_pct numeric DEFAULT 10
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER AS $fn$
DECLARE
  v_role text; v_config_id uuid; v_fund_alloc_id uuid;
  v_instance_id uuid; v_initial_status text; v_scheduled_end timestamptz;
BEGIN
  SELECT role INTO v_role FROM users WHERE id = p_admin_id;
  IF v_role NOT IN ('admin', 'super_admin') THEN RAISE EXCEPTION 'Unauthorized: not an admin'; END IF;
  INSERT INTO auction_configs (
    name, slug, auction_type, bid_fee, currency, min_bid_value, max_bid_value, image_url,
    total_bids_to_hot, hot_mode_duration_seconds, auction_duration_seconds, total_bids_to_close,
    prize_type, manual_prize_title, jackpot_seed, prize_pool_pct, platform_pct, burn_pct,
    social_circle_pct, rollover_pct, created_by, is_template
  ) VALUES (
    p_name, p_slug, p_auction_type, p_bid_fee, 'PNGWIN', p_min_bid_value, p_max_bid_value, p_image_url,
    p_total_bids_to_hot, p_hot_mode_duration_seconds, p_duration_seconds, p_total_bids_to_close,
    p_prize_type, p_manual_prize_title, p_jackpot_seed, p_split_prize_pct, p_split_house_pct, p_split_burn_pct,
    p_split_social_pct, p_split_jackpot_pct, p_admin_id, false
  ) RETURNING id INTO v_config_id;
  INSERT INTO fund_allocations (
    game_instance_id, game_type, prize_pool_pct, house_fee_pct, social_circle_pct,
    jackpot_pct, burn_pct, entry_cost, currency
  ) VALUES (
    gen_random_uuid(), 'auction', p_split_prize_pct, p_split_house_pct, p_split_social_pct,
  ...
  INSERT INTO auction_instances (
    config_id, fund_allocation_id, status, total_bids, unique_bidders, total_bid_fees,
    prize_pool, burned_amount, scheduled_end
  ) VALUES (
    v_config_id, v_fund_alloc_id, v_initial_status, 0, 0, 0,
...
serve(async (req) => {
  ...
  try {
    ...
    const statements = [
      { name: 'split_funds', query: SQL_SPLIT_FUNDS },
      { name: 'place_auction_bid', query: SQL_PLACE_AUCTION_BID },
      { name: 'admin_create_auction', query: SQL_ADMIN_CREATE_AUCTION },
    ];
    ...
    for (const stmt of statements) {
      try {
        await sql.unsafe(stmt.query);
        results.push({ name: stmt.name, success: true });
      } catch (e) {
        results.push({ name: stmt.name, success: false, error: e.message });
      }
    }
